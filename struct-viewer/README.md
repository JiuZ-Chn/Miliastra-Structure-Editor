...
- **structId 需手动填写**：导入或新建结构体定义后不会自动生成 structId，需在编辑器中手动填写（仅数字）；未填写时会红框高亮提示。
- **变量绑定定义**：导入结构体变量时，当前存档必须已存在相同 `structId` 的结构体定义，否则会拒绝导入；变量的 structId 与字段名均来自其绑定的定义，不可单独修改。
